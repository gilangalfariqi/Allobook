...
CMD ["sh", "-c", "timeout 30s pnpm --filter @allobook/api run db:deploy || echo 'Migration failed or timed out, starting server anyway'; (sleep 5 && cd apps/api && node dist/seed.js) & pnpm --filter @allobook/api start"]
